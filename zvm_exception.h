...
static const zvm_exception_entry_t exception_handlers [] = {
    ...
    ZVM_EXCEPTION_HANDLER_ENTRY(DECODE,  BAD_INSTRUCTION,  bad_instruction,  "decoding:ß bad instruction error")
    ZVM_EXCEPTION_HANDLER_ENTRY(EXECUTE,  BAD_MEMORY_ADDRESS,  bad_memory_address,  "excuting:ß bad memory address error")
    ZVM_EXCEPTION_HANDLER_ENTRY(EXECUTE,  STACK_OVERFLOW ,  stack_overflow,  "excuting:ß stack overflow error")
    ZVM_EXCEPTION_HANDLER_ENTRY(EXECUTE,  STACK_UNDERFLOW ,  stack_underflow,  "excuting:ß stack underflow error")
